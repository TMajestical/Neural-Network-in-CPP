...
class Network
{
        ...
        public:
            ...
            void backprop(vector<vector<vector<double>>> &all_Ais,vector<vector<vector<double>>> &all_His,double y_true,vector<double> &y_pred, vector<vector<vector<double>>> &dw_list,vector<vector<double>> &db_list)
            {
                ...
                auto dw_size = dw_list.size();
                auto db_size = db_list.size();
                ...
                dw_list.reserve(dw_size);
                db_list.reserve(db_size);
                ...
                {
                    dw_list.emplace_back(Outer(all_His[i],grad_ai));
                    db_list.emplace_back(grad_ai); //gradient of biases = grad_ai
...
class Optimiser
{
    public:
        ...
        void rmsprop(vector<vector<double>> &X_train,vector<double> &Y_train,vector<vector<double>> &X_valid,vector<double> &Y_valid,Network &model,double lr,int epochs,int batch_size,double l2_param)
        {
            ...
            //vector<vector<vector<double>>> dw_cur(model.layers.size());
            //vector<vector<double>> db_cur(model.layers.size());
            ...
            for(int epoch = 0;epoch<epochs;epoch++)
            {
                ...
                for(int zz=0;zz<X_train.size();zz++) //just a dummy variable to loop over the train data
                {
                    ...
                    /*cout<<"Computed Weight Gradients shape : \n";
                    model.printDims(dw_cur);
                    cout<<"Existing Weight Gradients shape : \n";
                    model.printDims(dw);

                    cout<<"Computed Bias Gradients shape : \n";
                    model.printDims(db_cur);
                    cout<<"Existing Bias Gradients shape : \n";
                    model.printDims(db_cur);*/
